...
SAMPLE_BOUNDX = np.trunc((XLEN-1)/5)
SAMPLE_BOUNDY = np.trunc((XLEN-1)/5)
...
for ticks in ticks_vec:
    for g in range(SAMPLES):
        if tick_index == 0:
            new_map.set_mcl_cell(samples_x[g], samples_y[g], 1)
 ...
 #------ MCL Model -------#
        ...
        # -----------------------------------
        grid_data = new_map.get_grid(sample_x, sample_y)
        gridx = grid_data[0]
        gridy = grid_data[1]
        ...
        for s in range_distances:
            if s <= 50:
                ...
                new_map.set_mcl_cell(s_data[0], s_data[1], 0.25)
                
            range_pos = range_pos + 9

        samples_p[g] = q
        sample_p = q
        new_map.set_mcl_cell(gridx, gridy, q)
    ...
    X_sample = mr.low_variance_sampler(Xt, samples_p)
    resamples_x = []
    resamples_y = []
    resamples_t = []
    resamples_p = []
    print("Re-sampling")
    for i in range(SAMPLES):
        resamples_x.append(samples_x[X_sample[i]])
        resamples_y.append(samples_y[X_sample[i]])
        resamples_t.append(samples_t[X_sample[i]])
        resamples_p.append(samples_p[X_sample[i]])
...
for x in range(XLEN):
    for y in range(YLEN):
        if file_conn != None:
            file_conn.write(f'{new_map.mcl_matrix[x,y]} ')
    file_conn.write("\n")
file_conn.close()